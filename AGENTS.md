...
This section is for automated cloud agents. The VM is Ubuntu
24.04 (matching `devops/deploy/docker/build/ubuntu24.04`). The
startup/update script already installs the apt build
dependencies and runs `git submodule update --init
--recursive`. Cloudberry is built natively (no Docker); this
diverges from the Docker-based flow in `devops/README.md`.

### Non-obvious gotchas

- Submodules are mandatory. `contrib/pax_storage` (PAX
  columnar AM, built via `--enable-pax`) depends on the
  `tabulate`, `googletest`, and `yyjson` submodules. Without
  `git submodule update --init --recursive` the build fails
  with `does not contain a CMakeLists.txt`. The update script
  handles this.
- Compiler selection. The base image's `cc`/`c++`
  alternatives may point at `clang`, which breaks
  `pax_storage`'s CMake step with `cannot find -lstdc++` (the
  autotools portion still uses `gcc`, so the failure only
  surfaces in the PAX CMake subproject). Always build with
  `CC=gcc CXX=g++` (cmake honors these), e.g.
  `CC=gcc CXX=g++ make -j$(nproc)`. Setup also points the
  `cc`/`c++` alternatives at GCC 13.
- `postgres`/`initdb` refuse to run as root. The cluster runs
  as the normal VM user (not root).
- The demo cluster needs a running `sshd` and passwordless SSH
  to `localhost` and `$(hostname)` (gpinitsystem/gpstart use
  ssh). The update script does NOT start services; if `ssh
  localhost` fails, start sshd (`sudo mkdir -p /run/sshd &&
  sudo /usr/sbin/sshd`) and add your key to
  `~/.ssh/authorized_keys`.

### Build and install (native)

Configure flags mirror
`devops/build/automation/cloudberry/scripts/configure-cloudberry.sh`
but drop the Rocky-only `--with-includes=/usr/local/xerces-c`
(Ubuntu uses the system `libxerces-c-dev`). Install prefix is
`/usr/local/cloudberry-db` (`BUILD_DESTINATION`).

```
export BUILD_DESTINATION=/usr/local/cloudberry-db
export LD_LIBRARY_PATH=$BUILD_DESTINATION/lib:$LD_LIBRARY_PATH
export CC=gcc CXX=g++
./configure --prefix=$BUILD_DESTINATION --disable-external-fts \
  --enable-gpcloud --enable-ic-proxy --enable-mapreduce \
  --enable-orafce --enable-orca --enable-pax --disable-pxf \
  --enable-tap-tests --with-diskquota --with-gp-stats-collector \
  --with-gssapi --with-ldap --with-libxml --with-lz4 \
  --with-openssl --with-zstd --with-pam --with-perl \
  --with-pgport=5432 --with-python --with-pythonsrc-ext \
  --with-ssl=openssl --with-uuid=e2fs --without-mdblocales \
...
### Run a local cluster (developer workflow)

```
source /usr/local/cloudberry-db/cloudberry-env.sh   # sets GPHOME, PATH, PYTHONPATH
make create-demo-cluster                            # coordinator + standby + 3 primary/3 mirror segs
source gpAux/gpdemo/gpdemo-env.sh                    # sets PGPORT=7000, COORDINATOR_DATA_DIRECTORY
gpstate                                              # cluster status
psql -p 7000 -d postgres                             # connect (user = current VM user)
```

Manage with `gpstart -a` / `gpstop -a`. Data dirs live under
`gpAux/gpdemo/datadirs/` (ignored by git).

### Tests and lint

- Regression suite uses `pg_regress` against the running demo
  cluster. Run a subset with
  `make -C src/test/regress installcheck-tests TESTS="..."`.
  Many tests depend on shared fixtures created by `test_setup`,
  so include `test_setup` first (e.g.
  `TESTS="test_setup int4 text"`). `pg_regress` does not clean
  cluster-global objects (tablespaces/roles) between runs; drop
  `regress_tblspace`, `mdb_admin`, `mdb_superuser` and the
  `regression` DB before re-running to avoid "already exists"
  noise. Full runs: `make installcheck-good` (large) or
  `make -C src/test installcheck-cbdb-parallel`.
- There is no standalone lint target. C/C++ static analysis is
  clang-tidy (config in `.clang-tidy`) plus Coverity/SonarQube
  in CI; license-header linting is the Apache RAT audit
  (`mvn apache-rat:check`, needs Maven/JDK). These are not part
  of the normal local dev loop.
