...
_discoverArchiveFormat(ArchiveHandle *AH)
{
	...
	else
	{
		...
		else if (AH->lookaheadLen >= strlen(GP_TEXT_DUMPALL_HEADER) &&
		         (strncmp(AH->lookahead, GP_TEXT_DUMP_HEADER, strlen(GP_TEXT_DUMP_HEADER)) == 0 ||
		          strncmp(AH->lookahead, GP_TEXT_DUMPALL_HEADER, strlen(GP_TEXT_DUMPALL_HEADER)) == 0))
		{
			/*
